# SEG-08 MiniMax H3 生成說明 v02

2026-09-21。依 `brief-v03.md`（使用者授權修訂）製作，使用者直接要求調整既有 H3；Storyboard v01 已過時、新版尚未製作。
...
- `seg08-a-minimax-h3-prompt-v02.txt`：承接 SEG-07 B v07 末端站姿；Cadi 把整片發光布局平面托離工作檯、平移到女主角面前並微微傾斜；女主角站著端詳，食指在布局上方空中筆畫（不觸碰平面），低聲咕噥 DLG-20；12–13秒持續對 placement 比劃。
...
| 編號 | 角色 | 內容 | 位置 |
|---|---|---|---|
| DLG-20 | 女主角 | 或許cpu往旁邊移動一點 | seg08-a 9–12秒，咕噥、低音量、句尾收弱；12–13秒持續比劃 |
...
依 `reference-manifest-v02.json`，兩段都是 `ref_image_0` = 女主角、`ref_image_1` = 紅焰 Cadi（依 registry priority 排序，heroine 10 在 cadi 20 之前）。工廠改採 `virtual-factory-description-v02.txt` 純文字場景。
...
`.agents/skills/h3-scene-prompt/scripts/check_prompt.py` 對兩段各執行一次，A／B 均為13項通過、0警告、0失敗；`project.visual_style` 與 heroine／cadi canonical 逐字一致，索引與 cast 相符。
