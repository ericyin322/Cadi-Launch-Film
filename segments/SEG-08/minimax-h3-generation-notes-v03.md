# SEG-08 MiniMax H3 生成說明 v03

2026-09-21。依 `brief-v03.md`（使用者授權修訂）製作；A 段依使用者要求將布局 Video 參考改為圖片參考，B v02 保持不變。Storyboard v01 已過時、新版尚未製作。
...
- `seg08-a-minimax-h3-prompt-v03.txt`：承接 SEG-07 B v07 末端站姿；Cadi 把整片發光布局平面托離工作檯、平移到女主角面前並微微傾斜；女主角站著端詳，食指在布局上方空中筆畫（不觸碰平面），低聲咕噥 DLG-20；12–13秒持續對 placement 比劃。布局改由 `<Picture 3>` 鎖定。
...
`.agents/skills/h3-scene-prompt/scripts/check_prompt.py` 驗證結果：A v03 為14項通過、0警告、0失敗；B v02 以 v03 registry 複驗為13項通過、0警告、0失敗。`project.visual_style`、heroine／cadi／layout_ref canonical、圖片索引與 cast 均一致。
