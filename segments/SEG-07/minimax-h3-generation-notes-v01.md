...
| 項目 | A：14秒 | B：12秒 |
|---|---|---|
| 提示詞 | seg07-a-minimax-h3-prompt-v01.txt | seg07-b-minimax-h3-prompt-v01.txt |
...
圖片預設路徑：
- 女主角：`assets/characters/heroine-photo-reference-v01.png`
- Cadi：`assets/characters/cadi Design Sheet.jpg`
...
`reference-manifest-v01.json` 包含完整圖片及影片綁定；根目錄 manifest.csv 僅記圖片順序。registry 的 reference_videos 欄另記影片。
...
成片畫風逐字取自主registry project.visual_style；Cadi沿用SEG06無女主角身高比較句的canonical，兩段逐字一致。聲音沿用SEG06的112 BPM輕快器樂、低街區底聲與輕微機械／投影聲；A全段無對白，B僅Cadi一句。參考影片不綁音訊。剪輯時用連續音樂底接合兩段。
...
- A 6757字元；B 6882字元。各六個連續beat，分別覆蓋14秒／12秒。
